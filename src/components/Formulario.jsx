...
const handleChange = (e) => { 
    ...
    const handleSubmit = async(e)=>{
        e.preventDefault()//Cuando se envie el formulario este no recargue 
        if (Object.values(form).includes("") || Object.entries(form).length === 0)
        {
            setError(true)
            setTimeout(() => {
                setError(false)
            }, 1000);//muestra el mensaje por un segundo 
            return
        }
        try {

            if(form.id){
                const url = `https://65b81b9146324d531d55f560.mockapi.io/metro/v1/metro/${form.id}`
                await fetch(url,{
                    method:'PUT',
                ...
                const url ="https://65b81b9146324d531d55f560.mockapi.io/metro/v1/metro"
                            form.id = uuidv4()
                await fetch(url,{
                    method:'POST',
                    body:JSON.stringify(form),
