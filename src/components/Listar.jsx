...
const Listar = ({ estado,setIdmetro }) => {
    ...
    useEffect(() => {
        if (estado || rutas.length >= 0) {
            (async function () {
                try {
                    const respuesta = await (await fetch("https://65b81b9146324d531d55f560.mockapi.io/metro/v1/metro")).json()
                    setRutas(respuesta)
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (confirmar) {
                const url = `https://65b81b9146324d531d55f560.mockapi.io/metro/v1/metro/${id}`
                await fetch(url, {
                    method: 'DELETE',
                })
